  ...
  describe("#execute", () => {
    describe("When execute the method", () => {
      it("returns products", async () => {
        const products = [Voucher.createVoucher({id: `aaa`, code: "#X321", type: "fixed", amount: createMoney(100)})];
    ...
    describe("When service is unavailable", () => {
        it("returns error", async () => {
          const products = [Voucher.createVoucher({id: `aaa`, code: "#X322", type: "percentual", amount: createMoney(100)})];
          const productRepo = new FakeVoucherRepository(products);
